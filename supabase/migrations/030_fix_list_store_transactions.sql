-- Fix transaction date filtering (inclusive local calendar days) and empty jsonb edge cases
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE (p_type IS NULL OR p_type = '' OR u.tx_type = p_type)
      AND (p_date_from IS NULL OR u.tx_date >= p_date_from::TIMESTAMPTZ)
      AND (p_date_to IS NULL OR u.tx_date < (p_date_to + INTERVAL '1 day')::TIMESTAMPTZ)
      AND (
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.tx_date DESC), '[]'::JSONB)
  INTO v_items
  FROM paged p;
