-- Aggregate base_qty per product before POS sale (multi-line / multi-unit cart)

CREATE OR REPLACE FUNCTION assert_pos_sale_stock(
  p_store_id UUID,
  p_items JSONB
) RETURNS JSONB AS $$
DECLARE
...
BEGIN
    ...
    SELECT
      ...
      SUM(
        COALESCE(
          x.base_qty,
          ROUND(
            COALESCE(x.sale_unit_qty, x.quantity, 0)
            * COALESCE(product_unit_conversion(x.product_id, x.sale_unit_id), 1),
            3
          )
        )
      ) AS total_base
  ...
  LOOP
    ...
    FROM products
    WHERE id = v_agg.product_id AND store_id = p_store_id
    FOR UPDATE;
    ...
    IF v_product.track_inventory AND v_product.stock_quantity < v_agg.total_base THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', format(
          'Insufficient stock for %s (need %s base units, have %s)',
          v_product.name,
          v_agg.total_base,
...
-- Patch stock pre-check in POS sale (replace per-line check with aggregated helper)
...
GRANT EXECUTE ON FUNCTION assert_pos_sale_stock(UUID, JSONB) TO authenticated;
